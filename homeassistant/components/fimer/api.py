"""API for ABB REST INVERTOR bound to Home Assistant OAuth."""
...
class ConfigEntryAuth(my_pypi_package.AbstractAuth):
    """Provide ABB REST INVERTOR authentication tied to an OAuth2 based config entry."""
    ...
    def __init__(
        ...
        """Initialize ABB REST INVERTOR Auth."""
        self.hass = hass
        self.session = oauth_session
        super().__init__(self.session.token)

    def refresh_tokens(self) -> str:
        """Refresh and return new ABB REST INVERTOR tokens using Home Assistant OAuth2 session."""
